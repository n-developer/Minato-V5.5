...
import telegramapi
from telegramapi import enums
from telegramapi.types import (
    Message,
    CallbackQuery,
    ChosenInlineResult,
    InlineQuery,
...
class Filter:
    async def __call__(self, client: "telegramapi.Client", update: Update):
...
def command(commands: Union[str, List[str]], prefixes: Union[str, List[str]] = "/", case_sensitive: bool = False):
    ...
    async def func(flt, client: telegramapi.Client, message: Message):
